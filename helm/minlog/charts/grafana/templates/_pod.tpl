...
{{- end}}
  - name: {{ .Chart.Name }}
    {{- if .Values.image.sha }}
    image: "{{ include "grafana.imagerepository" . }}:{{ .Values.image.tag | default .Chart.AppVersion }}@sha256:{{ .Values.image.sha }}"
    {{- else }}
    image: "{{ include "grafana.imagerepository" . }}:{{ .Values.image.tag | default .Chart.AppVersion }}"
    {{- end }}
...
volumes:
  ...
  - name: storage
    hostPath:
    {{- toYaml .Values.persistence.hostPath | nindent 6 }}
